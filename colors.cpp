...
#include "contours.h"
#include "colors.h"
#include "global_var.h"#pragma once
#include <cv.h>
#include <highgui.h>
#include <stdio.h>
#include <vector>
...
{
	if((ptr[3*x]>6&&ptr[3*x]<15)&&ptr[3*x+1]>190&&ptr[3*x+1]<=245&&ptr[3*x+2]>120&&ptr[3*x+2]<230)
		return true;
	else
		return false;
}
...
{	
    if((ptr[3*x]>95&&ptr[3*x]<120)&&(ptr[3*x+1]>60&&ptr[3*x+1]<=230)&&(ptr[3*x+2]>150&&ptr[3*x+2]<220))
        return true;
    else
        return false;
}
...
{
    if((ptr[3*x]>120&&ptr[3*x]<140)&&(ptr[3*x+1]>40&&ptr[3*x+1]<200)&&(ptr[3*x+2]>40&&ptr[3*x+2]<=210))
        return true;
    else
        return false;
}
...
inline bool check_orange(uchar *ptr,int x)
{
	if((ptr[3*x]>6&&ptr[3*x]<25)&&ptr[3*x+1]>190&&ptr[3*x+1]<=245&&ptr[3*x+2]>120&&ptr[3*x+2]<230)
		return true;
	else
		return false;
}
