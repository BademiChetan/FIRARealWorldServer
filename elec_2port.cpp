...
/***************Global Variables******************/
SerialPort *p0,*p1;                                                 
unsigned char bot_code[5][12];                                  // Matrix which acts like the semaphore
...
#ifdef FINAL
boost::mutex serial_buffer_lock;                                // Mutex for serial buffer
#endif
unsigned long int read_timeout=0;                               // Counter for timeouts
int ENSURE_TIMEOUT=5;                                           // No. of tries to ensure
unsigned char bot_port[]={0,1,0,0,1};                           // Assign the ports here
/************************************************************************************
...
char readB(bool port,unsigned int timeout);
void writeB(bool port,unsigned char byte);
int usart_init0();
int usart_init1();
void bot_status();
void extract(bool port);
void prelude(bool port);
void Uinit();
void Uend();
bool rxstring(char *cstr, unsigned int timeout_val, bool print_str, bool port);
...
/*************************************************
 * FUNCTION NAME: readB()
 * ARGUMENTS    : bool port             => 0 means port 0
 *                                         1 means port 1
 *                unsigned int timeout  =>timeout
 * DESCRIPTION  : ReadByte encapsulation
 * RETURN VALUES: The byte read
 ************************************************/
char readB(bool port, unsigned int timeout)
{
    if(port)
        return p1->ReadByte(timeout);
    else
        return p0->ReadByte(timeout);
}

/*************************************************
 * FUNCTION NAME: writeB()
 * ARGUMENTS    : bool port             => 0 means port 0
 *                                         1 means port 1
 *                unsigned char byte    => Byte to be written
 * DESCRIPTION  : WriteByte encapsulation
 ************************************************/
void writeB(bool port, unsigned char byte)
{
    if(port)
...
int usart_init0()
{
    int usart_fl=1;                                                 // Flag for usart_init
    cout<<"Initializing port 0..."<<endl;
    if(p0->IsOpen())
    ...
    else
    {
        while(usart_fl)
        {
            ...
            {
#ifdef HIGHS
                p0->Open(SerialPort::BAUD_115200,
...
int usart_init1()
{
    ...
    else
    {
        while(usart_fl)
        {
            ...
            {
#ifdef HIGHS
                p1->Open(SerialPort::BAUD_115200,
                        SerialPort::CHAR_SIZE_8,
                        SerialPort::PARITY_NONE,
                        SerialPort::StopBits(1),
                        SerialPort::FLOW_CONTROL_NONE
...
void bot_status()
{
    ...
    cout<<"ID  "<<"Conx  "<<"Status  "<<"Errors  "<<"Success "<<"Interrupted "<<"No_ack   "<<"Gen_pwm  "<<"Enc_cnt "<<"Turn_pwm "<<"Enc_cnt_turn "<<"Battery_Voltage"<<"Port"<<endl;
    for(int i=0;i<5;++i)
        cout<<i<<"     "<<bot_code[i][0]<<"      "<<bot_code[i][1]<<"\t    "<<int(bot_code[i][2])<<"\t    "<<int(bot_code[i][3])<<"\t       "<<int(bot_code[i][4])<<"\t"<<int(bot_code[i][5])<<"\t  "<<int(bot_code[i][6])<<"\t   "<<int(bot_code[i][7])<<"\t    "<<int(bot_code[i][8])<<"\t      "<<int(bot_code[i][9])<<"\t\t"<<int(bot_code[i][10])<<"\t\t"<<int(bot_code[i][11])<<endl;
    cout<<"**************************************************************************************************************************************"<<endl;
    if(TIMER)
...
void extract(bool port)
{
    ...
    do
    {
        ...
        do
        {
            timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                //Waiting for all connections to be made
        ...
        {
            timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);         //Bn received
        }
        do
        {
            timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);
        ...
        bot_code[botID][0]='x';
        bot_code[botID][1]='f';
        bot_code[botID][11]=bot_port[botID];
        for(int i=2;i<10;++i)
            bot_code[botID][i]=0;
    ...
    {
        timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                 //Connected to all bots.
...
void prelude(bool port)
{
    ...
    char bn,bi;
    int i=1,timedout=0;
    while(!rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port))
...
void Uinit()
{
    p0 = new SerialPort("/dev/ttyACM0");
    p1 = new SerialPort("/dev/ttyACM1");
    usart_init0();
    usart_init1();
    prelude(0);
    prelude(1);
 ...
 *                1. Time_out
 ************************************************/
bool rxstring(char *cstr, unsigned int len, bool print_str,bool port=0)
{
    char temp=0;
    int i=0;
#ifdef FINAL
    serial_buffer_lock.lock(); 
#endif
    while(temp!='\n')
    {
        try
...
int sendenccmd(int botID, char action, int value=0, unsigned char speed=0)
{
    ...
    else if(is_enc_cmd(action)==3)                                            // Display the encoder values
    {
        ...
        while(!((temp>='0')&&(temp<='4')))
        {
            writeB(port,botID+48);
            try
            {
                temp=readB(port,TIMEOUT_READ);
                if(STR_DEBUG)
        ...
        {
            timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                       // Waiting for 'R'
            if(timedout)
            {
                timecount++;
        ...
        {
            timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                               // Waiting for 'N' 
            if(timedout)
            {
                timecount++;
                if(bot_code[botID][1]=='n')
    ...
    else if(is_enc_cmd(action)==4)                              //Encoder based command the way bademi wants 
    {
        ...
        else
        {
            ...
            do
            {
                timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                // AP will send out the decision as a sync 
                if(timedout)
                {
                    timecount++;
                    if(bot_code[botID][1]=='n')
            ...
            {
                timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                       // Waiting for the mirrored cmd 
                if(timedout)
                {
                    timecount++;
                    if(bot_code[botID][1]=='n')
    ...
    else if(is_enc_cmd(action)==5)                              //Encoder based command hence value!=0
    {
        ...
        else
        {
            ...
            writeB(port,charval[0]);
            writeB(port,charval[1]);
            writeB(port,charval[2]);
            cout<<"Next cmd sent"<<endl;
            timecount=0;
#ifdef SEC
            while(((serial_buffer[0]!=recv)||(timedout))&&(timecount<NO_TIMEOUT))
    ...
    else                                                                      // NFS style commands
    {
        ...
        if(!bypass_normal_protocol)
        {
            ...
            {
                timedout=rxstring(serial_buffer,TIMEOUT_VAL,STR_DEBUG,port);                              // Waiting for the mirror 
                if(timedout)
                {
                    timecount++;
                    if(bot_code[botID][1]=='n')
 ...
 * ARGUMENTS    : int botID     => Bot no. to be checked
 * DESCRIPTION  : Called before executing a sendenccmd within sendenccmd
 *                Checks if the bot exists
 *                  if it does, then it tries to see if it is free or busy
 *                      if it is free, peace
 *                      if it is busy, listen once for any D packets. Otherwise
 *                      send an interrupting command
 *                      Do the above for a max of ENSURE_TIMEOUT times
 * RETURN VALUES: NONE
 ************************************************/
void ensure_bot_free(int botID)
{
